...
def test_image_slot_becomes_placeholder_shape_without_asset() -> None:
    ...
    assert image_elements[0]["type"] == "shape"
...
def test_unmatched_image_slots_keep_placeholder_shape() -> None:
    adapter = PresentonTemplateAdapter()
    slide = adapter.compile_slide(
        "title_description_image",
        title="Architecture",
