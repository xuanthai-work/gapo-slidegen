...
function PresentSlide({
  ...
  return (
    <div className={`present-slide${visible ? " present-slide--visible" : ""}`}>
      <SlideCanvas
...
function EditorWorkspace({ presentationId }: { presentationId: string | undefined }) {
  ...
  const [presentControlsVisible, setPresentControlsVisible] = useState(true);
  ...
  const presentControlsTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const presentBarHoverRef = useRef(false);
  ...
  async function flushPendingSave() {
    ...
    try {
      ...
      if (caught instanceof ApiError && caught.status === 409) {
        ...
        toast.error("Could not save presentation");
      }
    } finally {
  ...
  function showPresentControls() {
    setPresentControlsVisible(true);
    if (presentControlsTimerRef.current) clearTimeout(presentControlsTimerRef.current);
    if (!presentBarHoverRef.current) {
  ...
  async function exportPptx() {
    ...
    try {
      ...
      toast.error(caught instanceof Error ? caught.message : "Could not export the presentation.");
    } finally {
      setExporting(false);
    }
  }
  ...
  function applyOperation(operation: EditOperation): boolean {
    try {
      ...
      toast.error(caught instanceof Error ? caught.message : "Could not edit the presentation.");
      return false;
    }
  }
  ...
  async function uploadImage(file: File) {
    ...
    try {
      ...
      toast.error(caught instanceof ApiError ? caught.message : "Could not upload the image.");
    } finally {
  ...
  async function generateImage() {
    ...
    try {
      ...
      toast.error(caught instanceof ApiError ? caught.message : "Could not generate the image.");
    } finally {
  ...
  async function rewriteWithAI() {
    ...
    try {
      ...
      toast.error(caught instanceof ApiError ? caught.message : "Could not rewrite the slide content.");
    } finally {
  ...
  function renderElementActions() {
    ...
    return (
      <>
        ...
        <button className="button button--quiet delete-element" title="Delete (Del)" onClick={removeSelectedElement}>
  ...
  const commands = useMemo<CommandAction[]>(
    () => [
      ...
      },
      {
        ...
        action: () => setActiveSlideIndex((current) => Math.max(0, current - 1)),
      },
      {
        id: "next-slide",
        section: "Slide",
        ...
        action: () => setActiveSlideIndex((current) => Math.min(document.slides.length - 1, current + 1)),
      },
      {
        id: "undo",
        section: "Edit",
      ...
      },
      {
        ...
        icon: <Copy size={16} />,
        action: () => {
          if (selected) duplicateSelectedElement();
        },
      },
      {
        id: "delete-element",
        ...
        icon: <Trash size={16} />,
        action: () => {
          if (selectedElementId) removeSelectedElement();
        },
      },
      {
        id: "insert-text",
        ...
        <main className="editor-shell">
          <header className="topbar">
            <div className="topbar__leading">
              ...
              <span className="history-controls" aria-label="Edit history">
                <button className="icon-button" aria-label="Undo" title="Undo (Ctrl+Z)" disabled={!editor.canUndo} onClick={() => applyHistory("undo")}><ArrowCounterClockwise size={16} /></button>
                <button className="icon-button" aria-label="Redo" title="Redo (Ctrl+Shift+Z)" disabled={!editor.canRedo} onClick={() => applyHistory("redo")}><ArrowClockwise size={16} /></button>
              </span>
            </div>
          ...
          <section className="editor-grid">
            <aside className="filmstrip" aria-label="Slides">
              ...
              <div className="filmstrip__slides">
                {document.slides.map((item, index) => (
                  <div className="thumbnail-row" key={item.id}>
                    <button
                      className={`thumbnail u-lift${index === activeSlideIndex ? " thumbnail--active" : ""}`}
              ...
              <button className="button button--quiet add-slide" disabled={document.slides.length >= 30} onClick={addSlide}>
            ...
            <section className="workspace" aria-label="Slide canvas">
              <div className="insert-bar">
                ...
                <button className="button button--quiet" onClick={insertText}><TextT size={16} />Text</button>
                <button className="button button--quiet" onClick={insertShape}><Square size={16} />Shape</button>
                <input
                  ref={imageInputRef}
                ...
                />
                <button className="button button--quiet" disabled={uploadingImage} onClick={() => imageInputRef.current?.click()}>
            ...
            <aside className="properties">
              ...
              <div className="properties__content">
                ...
                selected ? (
                  ...
                  <div className="empty-state selection-empty">
                    <span className="empty-state__icon"><Sparkle size={24} /></span>
                    <h2 className="empty-state__heading">Select an element</h2>
                    <p className="empty-state__body">Choose an element on the slide to inspect its properties.</p>
                    <p className="palette-hint">Press Cmd/Ctrl + K for commands.</p>
                  </div>
                )
                )}
              </div>
          ...
          {presenting ? (
            <section className="present-mode" aria-label="Presentation mode" role="dialog" aria-modal="true" onMouseMove={showPresentControls}>
              <div className="present-stage">
                <PresentSlide key={activeSlideIndex} slide={slide} resolveAssetUrl={resolveAssetUrl} />
                <div
              ...
              <div
                className={`present-controls${presentControlsVisible ? "" : " present-mode--controls-hidden"}`}
                onMouseEnter={() => {
                  presentBarHoverRef.current = true;
                  showPresentControls();
                }}
                onMouseLeave={() => {
                  presentBarHoverRef.current = false;
                  showPresentControls();
                }}
              >
                <button
